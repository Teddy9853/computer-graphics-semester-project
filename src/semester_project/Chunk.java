...
public class Chunk {
    ...
    private int getRiverCenterX(int z) {
    double curve = Math.sin(z * 0.30) * 6.0;
    return (CHUNK_SIZE / 2) + (int) curve;
}

private int getRiverCarveAmount(int x, int z) {
    int riverCenterX = getRiverCenterX(z);
    int distanceFromRiver = Math.abs(x - riverCenterX);

    if (distanceFromRiver <= 1) {
        return RIVER_DEPTH; // deepest middle part
    } else if (distanceFromRiver <= RIVER_WIDTH) {
        return RIVER_DEPTH - 1; // shallow river bank
    }

    return 0; // not part of the river
}
    ...
    private void generateTerrain() {
        ...
        for (int x = 0; x < CHUNK_SIZE; x++) {
            for (int z = 0; z < CHUNK_SIZE; z++) {
                ...
                int riverCarveAmount = getRiverCarveAmount(x, z);
                boolean isRiver = riverCarveAmount > 0;

                if (isRiver) {
                height = Math.min(height, WATER_LEVEL - riverCarveAmount);
